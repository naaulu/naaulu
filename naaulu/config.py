...
def add_spatial_args(parser):
    ...
    parser.add_argument(
        "--buffer",
        help="buffer around the area of interest (e.g. 40km, 40E3)",
        type=str,
        default="40km",
    )
...
def parse_area(args):
    ...
    if args.buffer is not None and geometry is not None:
        buffer_m = naaulu.util.parse_distance(args.buffer)
        geometry = naaulu.geography.buffer_geometry(geometry, distance=buffer_m)
