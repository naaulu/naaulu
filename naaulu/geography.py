...
_admin0_cache: dict[str, shapely.geometry.MultiPolygon] = {}
_admin1_cache: dict[str, list] = {}
_admin2_cache: dict[str, list] = {}


def _load_gadm(iso3, level):
    """Download and cache a single GADM JSON file for a country and level.

    Returns the parsed JSON features list, or None on failure.
    """
    data_dir = naaulu.config.get_data_dir("gadm")
    json_path = os.path.join(data_dir, f"gadm41_{iso3}_{level}.json")

    if not os.path.exists(json_path):
        url = f"https://geodata.ucdavis.edu/gadm/gadm4.1/json/gadm41_{iso3}_{level}.json.zip"
        try:
            downloaded_path = naaulu.network.download(url, filename=f"gadm41_{iso3}_{level}.json.zip")
            ...
            logger.debug(f"Skipping {iso3} level {level}: {e}")
            return None
...
def _load_countries():
    ...
    filename = naaulu.config.get_bundled_data_path("ne_50m_admin_0_countries.geojson")
...
def _load_admin0(iso_codes):
    """Load GADM admin-0 boundaries for specified countries.

    Returns a dict mapping ISO3 code -> MultiPolygon.
    """
    result = {}
    for iso3 in iso_codes:
        if iso3 in _admin0_cache:
            result[iso3] = _admin0_cache[iso3]
            continue

        features = _load_gadm(iso3, 0)
        if features is None:
            continue
        ...
        if geoms:
            ...
            _admin0_cache[iso3] = geom
            result[iso3] = geom

    return result


def _load_admin1(iso_codes):
    """Load GADM admin-1 boundaries for specified countries.

    Returns a dict mapping ISO3 code -> list of shapely geometries.
    """
    result = {}
    for iso3 in iso_codes:
        if iso3 in _admin1_cache:
            result[iso3] = _admin1_cache[iso3]
            continue

        features = _load_gadm(iso3, 1)
        if features is None:
            continue
...
def get_admin0(geom, iso_codes):
    """
    Get GADM admin-0 boundaries for countries intersecting the geometry.

    Parameters:
    - geom: Shapely geometry to filter countries by intersection.
    - iso_codes: set of ISO3 codes to restrict to.

    Returns:
    - shapely.geometry.MultiPolygon of country boundaries.
    """
    admin0_data = _load_admin0(iso_codes)
    selected = []
    for iso, geom_obj in admin0_data.items():
        if isinstance(geom_obj, shapely.geometry.MultiPolygon):
            for poly in geom_obj.geoms:
                if geom is not None and not poly.intersects(geom):
                    continue
                selected.append(poly)
        else:
            if geom is not None and not geom_obj.intersects(geom):
                continue
            selected.append(geom_obj)
    return shapely.geometry.MultiPolygon(selected)


def get_admin1(geom, iso_codes):
    """
    Get admin-1 boundaries for countries intersecting the geometry.

    Parameters:
    - geom: Shapely geometry to filter admin-1 regions by intersection.
    - iso_codes: set of ISO3 codes to restrict to.

    Returns:
    - shapely.geometry.MultiPolygon of admin-1 regions.
    """
    admin1_data = _load_admin1(iso_codes)
    selected = []
    for iso in iso_codes:
        regions = admin1_data.get(iso, [])
        for region_geom in regions:
            if geom is not None and not region_geom.intersects(geom):
                continue
            selected.append(region_geom)
    return shapely.geometry.MultiPolygon(selected)
...
def _load_admin2(iso_codes):
    """Load GADM admin-2 boundaries for specified countries.

    Returns a dict mapping ISO3 code -> list of shapely geometries.
    """
    result = {}
    for iso3 in iso_codes:
        if iso3 in _admin2_cache:
            result[iso3] = _admin2_cache[iso3]
            continue

        features = _load_gadm(iso3, 2)
        if features is None:
            continue
        ...
        for feature in features:
            ...
            if isinstance(g, shapely.geometry.MultiPolygon):
                ...
                geoms.append(g)
        _admin2_cache[iso3] = geoms
        result[iso3] = geoms

    return result


def get_admin2(geom, iso_codes):
    """
    Get admin-2 boundaries for countries intersecting the geometry.

    Parameters:
    - geom: Shapely geometry to filter admin-2 regions by intersection.
    - iso_codes: set of ISO3 codes to restrict to.

    Returns:
    - shapely.geometry.MultiPolygon of admin-2 regions.
    """
    admin2_data = _load_admin2(iso_codes)
    selected = []
    for iso, geoms in admin2_data.items():
        for region_geom in geoms:
            if geom is not None and not region_geom.intersects(geom):
                continue
            selected.append(region_geom)
    return shapely.geometry.MultiPolygon(selected)
...
def buffer_geometry(geometry, distance):
    ...
    Uses an azimuthal equidistant projection centered on the geometry
    for accurate metric buffering.
    """
    ...
    aeqd_crs = CRS(
        f"+proj=aeqd +lat_0={centroid.y} +lon_0={centroid.x} +datum=WGS84"
    )
    wgs84_crs = CRS("EPSG:4326")
    to_aeqd = Transformer.from_crs(wgs84_crs, aeqd_crs, always_xy=True)
    to_wgs = Transformer.from_crs(aeqd_crs, wgs84_crs, always_xy=True)

    geom_aeqd = shapely.ops.transform(to_aeqd.transform, geometry)
    buffered_aeqd = geom_aeqd.buffer(distance)
    return shapely.ops.transform(to_wgs.transform, buffered_aeqd)
...
def radar_coverage(*, location, distance):
    ...
    if distance == 0:
        return shapely.geometry.Point(location)
