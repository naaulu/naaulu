...
def _naive_utc(value):
    """Normalize to tz-naive UTC, as xradar stores np.datetime64 times.

    Naive input is assumed UTC, matching the convention used elsewhere in
    this module. Equivalent to letting numpy do the conversion, but without
    triggering its "no explicit representation of timezones" warning.
    """
...
def get_volume(*, time, duration, wsi, min_angle, max_angle):
    ...
    for vol in sweeps:
        ok = True
        for name in vol.children:
            ds = vol[name].ds
            if ds is not None and "azimuth" in ds.sizes:
                az_count = ds.sizes["azimuth"]
                if az_count != 360:
                    logger.warning(f"{wsi} {name}: azimuth count is {az_count}, expected 360, discarding sweep")
                    ok = False
                    break
        if ok:
            valid.append(vol)
    ...
    return xradar.util.create_volume(
        sweeps=valid,
        time_coverage_start=_naive_utc(time - duration),
        time_coverage_end=_naive_utc(time),
        min_angle=min_angle,
        max_angle=max_angle,
    )
